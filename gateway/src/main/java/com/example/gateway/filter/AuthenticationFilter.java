  ...
  public GatewayFilter apply(Config config) {
    return ((exchange, chain) -> {
      if (validator.isSecure.test(exchange.getRequest())) {
        ...
        org.springframework.http.HttpCookie authCookie = exchange.getRequest().getCookies().get(HttpHeaders.AUTHORIZATION).get(0);
        org.springframework.http.HttpCookie refreshCookie = exchange.getRequest().getCookies().get("refresh").get(0);
        ...
        try {
          if (activeProfile.equals("test")) {
            ...
            if (response.getStatusCode() == HttpStatus.OK) {
              ...
              if (cookiesList != null) {
                List<HttpCookie> httpCookie = java.net.HttpCookie.parse(cookiesList.get(0));
